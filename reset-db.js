...
async function reset() {
    console.log('Resetting and ensuring database schema...');
    try {
        // Drop session table to avoid schema conflicts
        await pool.query('DROP TABLE IF EXISTS session CASCADE;');
        console.log('Old session table dropped.');
        ...
        console.log('SUCCESS: Database schema is clean and ready.');
        process.exit(0);
    } catch (err) {
        console.error('FAILED: Reset error:', err.message);
        process.exit(1);
    }
}
